...
(function($) {
	...
	function SlideToAbout(){
		...
		if($(window).width() > 767){
			$('.about-section').fullpage({
				anchors: ['company-desc','services','works'],
				sectionsColor: [],
				navigation: false,
	...
	function SlideToContact(){
		...
		if($(window).width() > 767){
			$('.contact-section').fullpage({
				anchors: ['contact-details','send-message','newsletter'],
				sectionsColor: [],
				navigation: false,
				navigationPosition: 'right',
				navigationTooltips: [],
	...
	function SlideToHome(){
		...
		$('nav a.to-left .arrow-label').html("more info");
		$('nav a.to-right').attr("data-direction","contact");
		$('nav a.to-right .arrow-label').html("contact us");
	}
	...
	$(window).load(function(e) {
		...
		var showContent = setTimeout(function(){
			...
			$("#wordrotator").wordsrotator({
    			...
    			words: ['we are<br><span class="highlight">roho squared</span>', 'we take<br><span class="highlight">awesome pictures</span>', 'we consult on<br><span class="highlight">social media</span>']  	//-- Array of words, it may contain HTML values
			});
